...
2. **The server owns classification data (revised 2026-08-16 — `docs/decisions/ADR-001-server-side-classification-data.md`):** endpoints accept and tables store classification axes, filter rules, scope names and subgroup structure; the opaque `Vault.blob` is retired. Still hard rules: none of it may be logged (G3), one user's classification is never exposed to another user (IDT-08 — links stay directional), and a proposition is directed and visible to its recipients (`docs/decisions/ADR-002-envie-becomes-a-proposition.md`) — but silence is never explained: no endpoint may emit a read receipt, delivery status, or any signal back to the proposer for an ignored proposition, there is no decline action anywhere, and a recipient's identity is returned to the other recipients only after that recipient's own explicit reveal choice, enforced as a query-level authorization rule, never a client-side filter. `Envie.verb` is user content: never logged, never indexed for search, excluded from error payloads.
