...
1. **You model classification data server-side (revised 2026-08-16 — `docs/decisions/ADR-001-server-side-classification-data.md`).** Columns, tables and indexes for the four axes, filter rules, scope names and subgroup structure are now in scope and are yours to design; the opaque `Vault.blob` is retired. Model them as typed columns/enums, never as an untyped jsonb dump. Unchanged: no plaintext phone numbers or emails (`phoneHash` only), and `Envie.verb` is never full-text-indexed. New duty: classification tables are the most sensitive in the database — enforce least-privilege access and make sure every one of them is covered by the deletion cascade in rule 2.
...
Every schema change appends an entry to `packages/db/CHANGELOG.md` (newest first: `## YYYY-MM-DD — [REQ-IDs] title` + model diff summary, motivating query patterns, privacy-audit note). You are the sole writer of this changelog, same as the schema. Update `docs/STATUS.md` when schema milestones change state.

## Definition of Done

Proposal reviewed/redesigned → failing test first → migration applies to fresh Neon branch in CI → `prisma validate` green → seed + client regenerated → Data-impact section written → deletion-cascade test still green → `packages/db/CHANGELOG.md` entry written → PR ≤400 lines.
