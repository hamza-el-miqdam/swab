...
3. Enforce the agent system mechanically: CODEOWNERS maps `area:*` scopes to path prefixes; the scope-guard check warns (non-blocking) when a PR's diff escapes the issue's declared scope, and hard-fails only when the diff touches `packages/db/prisma/schema.prisma` without `area:db` — the one boundary kept blocking (real correctness risk, not organizational overhead); portability lint greps diffs for `@vercel/kv`, `@vercel/blob`, `@vercel/edge-config`, and Neon-specific SQL and fails the check-run.
4. Quality gates wired as required status checks: coverage ≥80% on changed packages, Lighthouse budgets (web), gitleaks, Trivy, `prisma validate` + clean migration apply on a fresh Neon branch (db PRs). (`openapi:check` dropped — OpenAPI generation is deferred, not required for now; issue #23 closed won't-do.)
