...
Failing test first → implementation → 80% coverage → `openapi:check` green + client regenerated → new hot-path queries have an `area:db` index request filed with EXPLAIN evidence → PR ≤400 lines.
