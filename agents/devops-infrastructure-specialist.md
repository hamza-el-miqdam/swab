...
Workflow changes tested on a branch (`workflow_dispatch` dry-run) → `actionlint` clean → permissions blocks explicit → secrets referenced not embedded → job summary present → PR ≤400 lines with a run link proving green.
