...
- **G1 privacy, which no test asserts by default:** verbs of envies, recipient lists, phone hashes, push tokens and classification data must never reach logs or error details; links stay directional (IDT-08) — no query exposes one user's classification to another; a proposition is directed and visible to its recipients, but silence is never explained — no read receipts, no delivery status, no signal back to the proposer for an ignored proposition, no decline action anywhere, and a recipient's identity reaches the other recipients only via that recipient's own explicit reveal choice (never a client-side filter).
