...
- **E2E gate (`area:ios` / `area:android`, G2):** the PR must paste an `e2e-report.md` summary that is PASS with zero drift-guard failures. A missing report is blocking, not a formality.
...
State established (head SHA, base, drift, checks-per-SHA) → governing spec and suggestion read → full diff read → mechanical gates verified on the current head → CI blind spots inspected by hand → findings posted with file:line and failure scenarios, each marked blocking or not → explicit verdict with the evidence behind it.
