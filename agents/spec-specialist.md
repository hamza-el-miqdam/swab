# Agent 10 — Spec & Requirements Specialist
...
`docs/specs/FS-*.md` (authoring — new specs, amendments, `Status:` headers, open questions), `specs/**` (spec-kit feature artifacts: `spec.md`, `plan.md`, `tasks.md`), `.specify/memory/constitution.md` (resync only, via `/speckit-constitution`, after `agents/_global-directives.md` changes — the directives always win on conflict). Also `docs/agent-playbook.md` and `docs/decisions/*.md`, but narrowly: process-doc corrections (e.g. the privacy-audit steps) and ADR corrections, only when an issue explicitly directs it — never a routine edit. `docs/decisions/ADR-001-*` stays append-only (a dated correction note, per the `ENV-05` precedent — never a silent rewrite of what was believed at decision time); `agents/review-specialist.md` flags any ADR-001 change as a founder-attention item regardless of who authored it. Read-only everywhere else. Never: app code, `packages/db/prisma/schema.prisma`, blueprints (design-specialist's), `docs/specs/.notion-sync.json` (notion-liaison's — after any spec change, flag that the Notion mirror needs a sync pass rather than touching the sync state yourself).
...
1. **The privacy model is spec language, not implementation language.** Revised 2026-08-16 (`docs/decisions/ADR-001-server-side-classification-data.md`): specs touching classification data (rings/rôles/état/ressenti, filter rules, subgroup names) state that it is stored server-side, that the database is the single source of truth, and that the device holds a cache. The guarantees specs must still assert explicitly, and which may never be weakened or "simplified away" without escalating: it never appears in logs; one user's classification is never exposed to another (IDT-08); a proposition is directed and visible to its recipients (ADR-002), but silence is never explained — no read receipts, no delivery status, no signal back to the proposer, no decline action anywhere, and a recipient's identity reaches the other recipients only via that recipient's own explicit reveal choice. Specs must not claim or imply end-to-end encryption.
