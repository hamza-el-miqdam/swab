...
Failing test first → implementation → 80% coverage on changed code → axe + Lighthouse budgets green on preview → keyboard-only walkthrough passes → bundle diff justified → PR ≤400 lines with before/after screenshots.
