...
- Replaced `TRUST_PROXY_HOPS: number` (`env.ts`) with `TRUST_PROXY: string` — a comma-separated CIDR/IP allowlist, passed straight through to Fastify (which itself splits on commas and delegates to `@fastify/proxy-addr`). Fastify checks the *immediate peer's* address against the allowlist before trusting anything it says, closing the forgery — a hop count never could. Fail-closed default unchanged: unset → `false` → header always ignored.
- `app.ts:91` now does `trustProxy: deps.env.TRUST_PROXY ?? false` — the `?? false` (not a bare pass-through of `string | undefined`) is required under `exactOptionalPropertyTypes`; it also fixed the cascading `TS2345`/`TS2769` errors across the whole file that PR #158 was hitting, without needing to touch the fastify version pinned in `package.json` (still `^5.12.0`/5.12.0 installed) — confirmed both `pnpm --filter @repo/api build` and `typecheck` are clean at that version.
- `tests/auth.test.ts`'s `TRUST_PROXY_HOPS=1` case asserted the *unsafe* behavior (two forged XFF values got independent buckets) — rewritten using `app.inject()`'s `remoteAddress` option: a trusted-CIDR peer gets XFF honored, an untrusted peer's forged XFF is ignored outright (falls back to the real peer IP). `tests/env.test.ts`, `tests/helpers.ts` updated to match the new env shape; `.env.example` documents `TRUST_PROXY=10.0.0.0/8`.
- **Correction to the 2026-08-17 SUG-API-005 entry below:** its claim that a hop count is *"spoof-resistant unlike `trustProxy: true`"* was wrong — a hop count only checks *how many* forwarded entries exist, not *who* is presenting them, so a direct client could always forge enough hops to look proxied. Left uncorrected in place per G5 (history isn't rewritten); this entry is the record of the fix.
- No spec change needed: FS-07's IDT-03 only requires "throttled per phoneHash and per IP," not a specific trust mechanism. `suggestions/backend/SUG-API-005-trust-proxy-rate-limit.md` marked superseded (not deleted).
- **Review fix (same PR):** the initial `TRUST_PROXY` schema only rejected empty strings — a malformed CIDR (`"not-a-cidr"`) passed `loadEnv()` and only failed two layers down inside `@fastify/proxy-addr`, with a message that never names the variable. Added `isValidTrustProxyEntry`/`isValidTrustProxyList` in `env.ts` (mirrors fastify's own `split(',').map(trim)` parsing, accepts bare IPs, CIDR ranges, and the `loopback`/`linklocal`/`uniquelocal` presets `@fastify/proxy-addr` recognizes) so a typo now fails boot with a named, tested `env.ts` error instead of an unowned `TypeError`.
...
- Same idempotency/tombstone shape as `patchContact`/`deleteContact`: `runMutation()` reused verbatim, `not_found` for a missing/tombstoned parent contact never burns the mutation id, re-adding after a tombstone revives the row.
