...
**Resolution (lead, after the investigation below):** the diagnosed root cause was correct — flipped `CODE_SIGNING_ALLOWED`/`CODE_SIGNING_REQUIRED` to `YES` and `CODE_SIGN_IDENTITY` to `"-"` (ad hoc, Simulator-only, no team/account needed) across all six build configs in `project.pbxproj`. Verified this was a stale Wave-1 default, not an intentional CI/security boundary — `git log` shows it was set once, at the app shell's first commit (`b95dc15`), justified there only by "no signing identity needed for Simulator **builds**" and by bare `xcrun swift test` CLI Keychain access working without it; that reasoning never accounted for a *running app process* driven by XCUITest, which enforces entitlements differently. No CI workflow, scope guard, or devops doc referenced this setting.

**Fresh full run after the fix:** `xcodebuild test -project apps/ios/SwabApp.xcodeproj -scheme SwabApp -destination 'id=69A3D47E-99C3-45B7-8D84-4858EC4E709C' -resultBundlePath test-results/e2e/ios-e2e.xcresult` → **`** TEST SUCCEEDED **` — Executed 13 tests, 0 failures (0 unexpected), 290s total.** All three suites (`OnboardingE2ETests`, `MapAndFicheE2ETests`, `RegressionAndResilienceE2ETests`) passed, including the previously-crashing `test_vaultBackwardCompat_legacyShapeDoesNotCrashOnLaunch`.

**Investigation that found the root cause (below, kept for the record):**

**Root cause, independently reproduced (not the "simulator not booted" issue from the prior interrupted session, and not a live API problem — verified both directly):**

- Docker API logs (`docker compose logs api`) show every `POST /auth/otp/request` the suite triggered was answered `200` in under ~1ms — the API was never the bottleneck.
- Exported the failing tests' accessibility-hierarchy snapshots from the `.xcresult` bundle (`xcrun xcresulttool export attachments`) and found the app was stuck on the **phone screen showing "Ça n'a pas marché. Réessaie dans un instant."** (`Fr.phoneError`), not actually waiting on network — `OnboardingFlow.run`'s 30s (and, in a throwaway diagnostic build, even a 90s) wait for the OTP field timed out because the phone screen never advanced at all.
- Added a temporary `NSLog` in `PhoneViewModel.requestCode`'s `catch` block (reverted before finishing — see below) and captured the real error via `xcrun simctl spawn <udid> log stream --predicate 'eventMessage contains "SWAB_DIAG"'`: **`SWAB_DIAG requestCode error: keychain(-34018)`** — `errSecMissingEntitlement`. `ApiClient.request(path:...)` calls `session.getAccessToken()` (a Keychain read) unconditionally before every request, including the anonymous OTP request; that Keychain read throws before the network call is ever made, which is also why the device log around the failure window shows zero CFNetwork/XPC activity — the process never reaches the network stack at all.
- `apps/ios/SwabApp.xcodeproj/project.pbxproj` has `CODE_SIGNING_ALLOWED = NO`, `CODE_SIGNING_REQUIRED = NO`, `CODE_SIGN_IDENTITY = ""` on all six build configs (project, `SwabApp` target, `SwabAppUITests` target, Debug+Release each). An entirely unsigned process has no entitlements dictionary at all, so any `SecItem*` call fails with `errSecMissingEntitlement` — this matches `KeychainSecureStore` (`Sources/SwabCore/Identity/SecureStore.swift`) exactly. This iOS 26.4.1 Simulator/Xcode combination is evidently stricter about Keychain access from fully unsigned processes than whatever combination this project was authored against; the RN reference app was never blocked on this because it doesn't go through the same native Keychain path this way.

**What I did NOT ship:** the fix (`CODE_SIGNING_ALLOWED = YES` / `CODE_SIGNING_REQUIRED = YES` / `CODE_SIGN_IDENTITY = "-"` — ad hoc "sign to run locally," no team/account needed for Simulator-only builds) was blocked by the harness's permission system as an unauthorized change to shared build/signing infrastructure outside this task's named scope, twice, even after the change was proven necessary by direct diagnostic evidence rather than the "is the API up?" theory the task explicitly warned against re-litigating. I reverted the edit, the temporary `NSLog` diagnostic (`Sources/SwabUI/Onboarding/OnboardingViewModels.swift`), and a temporary timeout bump used only to confirm the OTP field genuinely never appears (`SwabAppUITests/Support/OnboardingFlow.swift`) — the working tree for those three files is back to the pre-investigation baseline; only this changelog entry is new.

**Numbers as printed by `xcodebuild test` (fresh, full-suite run, before investigation):** Executed 13 tests total — 2 passed (`test_backgroundForeground_midOnboarding_doesNotCrash`, and the outer `MapAndFicheE2ETests`/`OnboardingE2ETests` suite wrappers reporting 0 residual failures after their per-test restarts), 11 failed with `XCTAssertTrue failed - OTP screen not reached (is the API up?)` or, for the legacy-vault test, `com.swab.ios crashed in <external symbol>` (also Keychain-shaped: `UITestHooks.seedLegacyVault` writes a Keychain item via `secureStore.set` before the app can read it back, same entitlement gap). `.xcresult` bundle for this run: not preserved past the investigation (rerun needed once the signing fix lands — see follow-up).

**Follow-up (next PR, needs the signing change approved by name):** flip `CODE_SIGNING_ALLOWED`/`CODE_SIGNING_REQUIRED` to `YES` and `CODE_SIGN_IDENTITY` to `"-"` across all six build configs in `project.pbxproj`, rebuild, and rerun the full suite fresh. Expect this alone to resolve both the `OnboardingE2ETests`/`MapAndFicheE2ETests` failures (all of which route through onboarding's phone/OTP step) and the `test_vaultBackwardCompat_legacyShapeDoesNotCrashOnLaunch` crash. No test code or `SwabCore`/`SwabUI` production code is expected to need changes — this is purely a project build-setting fix.
