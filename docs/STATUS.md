...
| CI | 🟡 | `ci.yml` skeleton exists. `scope-guard.yml` now enforces `area:*` scope on PR diffs (warn-and-pass while unlabeled, not yet a required check). `postgres:17` service container added with `prisma migrate deploy` + `DATABASE_URL` wired through (SUG-OPS-013). Missing: privacy-audit job, coverage enforcement, OpenAPI diff gate, native E2E workflow (macOS + emulator runners — filed follow-up). |
