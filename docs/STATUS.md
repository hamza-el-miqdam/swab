...
| Database schema v0.1 | 🟡 | `users`; FS-05 envie/match models + seed; ADR-001 classification columns on `contact_links`/`contact_roles` + `client_mutations` ledger (2026-08-17). `vaults` deprecated, not dropped. `syncSeq` bigserial on `contact_links`/`contact_roles` (#168, 2026-08-30) — [#189](https://github.com/hamza-el-miqdam/swab/issues/189) moves `cursor.ts` onto it; blocked until `sync_seq` advances on UPDATE (INSERT-only default today). **FLT-01..08:** FilterRule model + seed + XOR check + partial unique indexes (2026-09-05). |
