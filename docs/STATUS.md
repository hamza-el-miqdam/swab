...
| FS-04 | Subgroups (FCA) | ⚪ Not started | Mobile | **[ADR-002](decisions/ADR-002-envie-becomes-a-proposition.md) — amended, not rewritten.** Groups stay **private to their owner** (privacy requirements survive verbatim) but become manual, with CRUD; FCA is demoted to an opt-in suggestion. « Tu ne définis jamais un groupe à la main » is void. |
| FS-05 | Envie & Match | ⚪ Not started | Mobile + Backend | **[ADR-002](decisions/ADR-002-envie-becomes-a-proposition.md) — full rewrite required, new requirement IDs.** An envie is now a directed, visible proposition answered by accept / counter-propose / ignore; accepting reveals your identity to the other recipients only if you choose. The matching engine will not be built. Blocked until `agents/_global-directives.md` G1(d) is amended — see [ROADMAP.md](ROADMAP.md) Phase 0b. OQ-PRO-6 (how a group converges without counters) must be answered before authoring. |
