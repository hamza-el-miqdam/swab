...
| Local dev stack | 🟢 | `docker compose up --build` → Postgres :5432, API :3001, Adminer :8080. API boot now runs `prisma migrate deploy` (was `db push`). Fixed 2026-09-05: broken Postgres volume from a failed init was recreated (see `packages/db/CHANGELOG.md` 2026-09-05). |
