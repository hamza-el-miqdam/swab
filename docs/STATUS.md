...
| FS-06 | Standing personal boundaries (veto absolu) | ⚪ Not started | Mobile + Backend | **[ADR-002](decisions/ADR-002-envie-becomes-a-proposition.md) OQ-PRO-7 — decided (B), narrowed 2026-09-13.** The rule-level/priority machinery is retired; `FLT-02` (veto absolu) survives verbatim as a per-contact boundary, enforced server-side (`FLT-09`). See [FS-06](specs/FS-06-filtering.md). |
