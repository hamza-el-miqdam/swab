...
| Database schema v0.1 | 🟡 | `users`; FS-05 envie/match models (`envies`, `envie_recipients`, `matches`, `proposals`) + seed; ADR-001 classification columns on `contact_links`/`contact_roles` + `client_mutations` ledger (2026-08-17). `vaults` deprecated, not dropped. `syncSeq` monotonic bigserial added to `contact_links`/`contact_roles` (#168, 2026-08-30) — `area:api` follow-up still open to rewrite `cursor.ts` onto it. **FLT-01..08:** FilterRule model + seed + XOR check + partial unique indexes (2026-09-05). |
