...
|---|---|---|---|---|
| FS-07 | Identity & Vault | 🟡 In progress | Backend | Auth (phone-OTP, JWT sessions), `/health` + `/ready`. **ADR-001 stage 3 slice 1 landed 2026-08-22** — typed `/contacts` CRUD + cursor delta pull (VLT-02/07/08/09); `Vault` deprecated, still served. Next: roles, rules, subgroups, history → stage 4 clients. Missing: refresh rotation (IDT-02), deletion (IDT-04), discovery (IDT-06), invites (IDT-07/09). |
...
| Mobile E2E gate (Wave 4) | 🟢 | Local, agent-enforced DoD gate — `scripts/e2e-{ios,android}.sh` → `test-results/e2e/e2e-report.md` + requirement manifest in `docs/qa/`. Not yet wired into CI. Android: MAP-01/MAP-09's `RelationshipMapE2ETest` rows and SUG-DES-011's `FicheTouchTargetsTest` are known-flaky under `ANDROIDX_TEST_ORCHESTRATOR` (issue #128, IDT-03 OTP rate-limit interaction) — MAP-01/MAP-09 reclassified `manual` in `e2e-coverage.json` pending confirmed-deterministic reruns. |
