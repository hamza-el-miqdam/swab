...
| Database schema v0.1 | 🟡 | `users`; FS-05 envie/match models + seed; ADR-001 classification columns on `contact_links`/`contact_roles` + `client_mutations` ledger (2026-08-17). `vaults` deprecated, not dropped. `syncSeq` on both tables (#168) — INSERT-only today; #189 moves `cursor.ts` onto it once it also advances on UPDATE. **FLT-01..08:** FilterRule model + seed + XOR check + partial unique indexes (2026-09-05). |
