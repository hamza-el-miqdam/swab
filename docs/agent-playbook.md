...
2. **Wire audit (revised 2026-08-26, #116 — ADR-001).** ⚠️ **Transitional (ADR-001 stage 3 → stage 4).** *Current, and what to actually audit today:* `POST/GET /vault` is still registered (`apps/api/src/app.ts`) and is what both mobile clients call every sync (e.g. `apps/android/.../vault/VaultSync.kt`) — classification data still travels inside its opaque blob. The typed `/contacts` endpoints exist server-side (ADR-001 stage 3) but have zero production call sites in either mobile client yet, so there is no `/contacts` traffic to inspect yet. Record all app traffic through a full user journey against the live `/vault` traffic and verify: (a) the vault payload is only ever sent to/returned from the authenticated owner's own endpoint, never in a response addressed to another user (IDT-08 — links stay directional); (b) a reveal never appears in a response unless both sides' envies matched — no one-sided envie leaks (ENV-11); (c) the G3 forbidden list holds on the wire too — no verbs, recipient lists, phone hashes, or push tokens outside their intended single-owner response; (d) phone numbers appear only as client-side salted hashes, never raw (IDT-01). *After stage 4 (client cutover — `/vault` removed from `apps/api/src/app.ts` and both mobile clients):* switch this audit to record traffic against the typed `/contacts` endpoints instead, and verify the same (a)–(d) checks against their typed fields directly. Update this step and `docs/qa/e2e-coverage.json` together in the stage-4 migration PR.
3. **Log audit:** grep structured logs for verbs, recipient lists, phone hashes, tokens (G3 forbidden list) — zero hits.
4. **Non-observability audit:** the `PRO-11` and `PRO-23` tests (ignoring a proposition is unobservable; every recipient's view stays bit-identical to any other recipient's actions) pass with response-shape equality.
