...
| FS-06 | **Resolved 2026-09-13 (OQ-PRO-7, outcome B) — narrowed, not deleted.** The rule-level/priority machinery that silently pruned a broadcast scope is retired; `FLT-02` (veto absolu) survives verbatim as a standing per-contact boundary. See [FS-06](../specs/FS-06-filtering.md). |
...
| OQ-PRO-7 | Does FS-06 survive at all? | **(B) Narrowed to standing personal boundaries.** `FLT-02` (veto absolu) survives, reworded to stand alone under the current model; the rule-level/priority machinery and the pre-send "Filtrés par tes règles" column are retired. Executed per [SUG-SPEC-018](../../suggestions/done/specs/SUG-SPEC-018-adr002-fs06-survival.md) outcome (B). *(Executed 2026-09-13 — see [FS-06](../specs/FS-06-filtering.md).)* |
