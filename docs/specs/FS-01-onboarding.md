...
| ONB-04 | Calibration is radial: « moi » center; dragging/tapping a contact assigns it to an intimacy ring (ring enumeration: see OQ-ONB-1 — blueprint and shipped apps disagree, unresolved). The layout must visually prefigure the FS-02 map. |
...
OQ-ONB-1 (**blocking for ONB-04's normative ring enumeration**): the Onboarding blueprint (`blueprints/swab - Onboarding (standalone) (1).html`, `INTIMACY` constant) defines **5** intimacy rings with labels `intime` / `proche` / `ami` / `lien faible` / `connaissance` (radii 34/55/78/102/130 — a single fixed constant, independent of the blueprint's unrelated treatment-A/B toggle). Both shipped native apps instead implement **4** rings with different labels — `Très proche` / `Proche` / `Familier` / `Plus loin` — sourced identically in `apps/ios/Sources/SwabCore/L10n/Fr.swift:153-156` (consumed by `apps/ios/Sources/SwabUI/Onboarding/CalibrateView.swift`) and `apps/android/app/src/main/kotlin/com/swab/android/l10n/Fr.kt:59-62` (consumed by `apps/android/app/src/main/kotlin/com/swab/android/ui/onboarding/CalibrateScreen.kt`). iOS and Android agree with each other — this is not a cross-platform bug — but neither matches the blueprint's count or copy, so the blueprint cannot be frozen into ONB-04 as-is without contradicting what both platforms already ship and test against (the known rings-3/4 `CalibrateScreen` text-wrap bug, `docs/STATUS.md`, already assumes N=4). Per spec standard #3, French copy must come from the blueprint verbatim, not be invented from app code — so this OQ records the de-facto shipped N=4 without asserting the app labels as frozen copy, and product must decide: update the blueprint to match ship, retrofit both apps to the blueprint's 5-ring model, or formally re-freeze the shipped 4-ring labels as the new blueprint. Resolve before FS-04's SGR-01 (shared ring enumeration, SUG-SPEC-005 test vectors) depends on a fixed value. (Identity assumptions tracked in product-overview §6.)
