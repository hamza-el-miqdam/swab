...
| FCH-04 | ⚠️ **Transitional (ADR-001).** History feed shows axis changes and relationship events (acceptances with this person, at coarse grain) over 12 months, newest first (relationship events per `PRO-25`, `docs/specs/FS-05-envie-match.md` — grain `{date, category}`, never the verb; deferred until FS-05's proposition model is implemented, `area:db` issue #183). *Current, and what the shipped code does:* the feed is read from the on-device vault, and the 12-month window is enforced by an **explicit interim device-side trim** — `Vault.recordAxisEdit` prunes `history` to 12 months at write time on both platforms (Android: PR #108, SUG-AND-013; iOS: PR #107) — landed as a deliberate stopgap, founder decision 2026-08-21 (issue #110), because the vault's 1MB per-user cap (`MAX_VAULT_BYTES`, `apps/api/src/routes/vault.ts:13`, plus a DB `CHECK` in `packages/db` migration `20260818220000`) is live today and unbounded history would eventually breach it and silently break vault sync. The trim does not trust the device clock alone: both platforms corroborate the cutoff against the newest stored history event before pruning, so a device clock running far enough ahead of "now" skips the prune instead of deleting every event for every contact — guarding against `VaultSync` faithfully re-pushing a clock-skew-induced wipe to the server (Android had this guard from the start, #108; iOS added it via #137, closing issue #113). *After the ADR-001 migration:* the feed becomes **server-stored and read through the cache**, and the 12-month window becomes a genuine server-side retention policy. *Removal condition:* the device-side trim on both platforms is deleted once that server-side retention policy ships as part of ADR-001 Stage 3 (the history slice) — tracked in issue #110, still open for that code-removal work. Running both trims at once once the server policy lands would mean two systems pruning the same data on different clocks — a correctness risk, not just redundancy. |
...
| FCH-06 | État values include at least the blueprint-attested `en pause`. ~~The fiche shows the FS-06 filter consequence for the current état (e.g., "en pause → exclu par défaut à l'envoi") so filtering stays legible.~~ **VOID 2026-09-16** (issue #201): FS-06's 2026-09-13 narrowing (ADR-002 `OQ-PRO-7` outcome B) retired `FLT-01`, the case-based état/ressenti default-rule system this clause depended on — see `docs/specs/FS-06-filtering.md`'s retired-`FLT-01` row. État now carries **no automatic filtering consequence of any kind**; the only filter left is `FLT-02` veto absolu, a manual boolean the owner sets once per contact from the contact card, entirely independent of état/ressenti (FS-06). The fiche must not display any état-derived "consequence" text — there is no truthful consequence left to surface, and showing one asserts a product behavior that no longer exists. The surviving clause of this requirement is only that `en pause` remains a valid, displayed état value. |
