...
| ID | Requirement |
|---|---|
| ENV-13 | Match surface **in the OPEN state with no pending incoming proposal** offers exactly: **Proposer un lieu**, **Proposer une heure**, **Passer cette fois** — copy per blueprint (« Vous voulez vous proposer un truc ? »). **With a pending incoming proposal**, the surface instead offers exactly TWO actions: accept, and **Passer cette fois** — there is no separate "decline" action (OQ-ENV-5, resolved: swab has no refusal philosophy distinct from the soft pass — pass IS the decline mechanism, product-wide, per `docs/product-overview.md`'s glossary and product law 5; declining this specific proposal and passing on the match are the same user-facing action, carrying ENV-15's silent/bit-identical-to-counterpart semantics). Accept button copy is ⚠️ MISSING COPY (OQ-ENV-4) — not present in any blueprint or spec; do not invent, French copy must come from the blueprints/specs verbatim (playbook §4 rule 5). |
