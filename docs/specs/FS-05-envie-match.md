...
| ID | Requirement |
|---|---|
| ENV-13 | Match surface **in the OPEN state with no pending incoming proposal** offers exactly: **Proposer un lieu**, **Proposer une heure**, **Passer cette fois** — copy per blueprint (« Vous voulez vous proposer un truc ? »). **With a pending incoming proposal**, the surface instead offers exactly: accept, decline, and — ⚠️ PROPOSED ASSUMPTION pending Hamza's confirmation (OQ-ENV-5) — **Passer cette fois** (assumed to stay available: a graceful exit must always exist, product law). Accept/decline button copy is ⚠️ MISSING COPY (OQ-ENV-4) — not present in any blueprint or spec; do not invent, French copy must come from the blueprints/specs verbatim (playbook §4 rule 5). |
| ENV-14 | Proposals (place and/or time) go to the counterpart, who accepts/declines; accepted → match state SCHEDULED. Simple single-proposal loop for POC — no negotiation threads. A proposal MUST carry at least one of {place, timeslot}; the API rejects an empty proposal (`422`). |
...
OQ-ENV-4: ENV-13's accept/decline actions (shown on a match with a pending incoming proposal) have no French button copy anywhere in the blueprints or specs — the "Flux envie et match" blueprint only covers the no-proposal state (« Vous voulez vous proposer un truc ? », **Proposer un lieu**, **Proposer une heure**). Per playbook §4 rule 5, missing copy is escalated, never invented. Design-specialist supplies frozen copy; this spec freezes it verbatim once received.
OQ-ENV-5: does **Passer cette fois** remain available on the match surface while a proposal is pending (ENV-13)? This spec currently assumes yes (a graceful exit must always exist — product law), as a ⚠️ PROPOSED ASSUMPTION, not a settled decision — Architect confirms with Hamza before implementation relies on it.
