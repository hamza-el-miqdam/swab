...
| ENV-05 | Scope→recipients resolution runs **on-device**. The server stores filter rules (FLT-06) but not subgroup *membership* — the lattice is derived on-device and never persisted (SGR-07, OQ-SGR-2) — so the server cannot resolve a portée on its own. The client resolves scope → members, applies filters, and sends the final recipient ID list, which the server validates (ENV-17) and treats as authoritative. Neither scope names nor filter reasons are ever disclosed to recipients or to any other user (IDT-08). *(Corrected 2026-08-16: an earlier ADR-001 edit said resolution MAY run server-side "now that subgroups are stored there" — that over-generalised from filter rules to membership, which is not stored.)* |
