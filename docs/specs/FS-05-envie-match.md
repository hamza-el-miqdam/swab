...
| ID | Requirement |
|---|---|
| PRO-01 | Free-text verb input, present-tense framing (« envie de… »), ≤ 200 chars. |
| PRO-02 | The recipient list is the only target type. **Individual is the base case** — a proposer may address one person directly with no group involved. A saved FS-04 group is an optional convenience for populating the same list at once; it is not a separate API shape, and the server resolves it into the identical `recipientIds` shape `PRO-23` validates (OQ-PRO-11). |
| PRO-03 | A lightweight `category` is kept, **narrowly**: it groups/filters the recipient's own relationship-history view (FCH-04, `docs/specs/FS-03-contact-card.md`) only. It is never shown to recipients, never used for delivery or eligibility, and never displayed alongside the proposition itself (OQ-PRO-5). ⚠️ PROPOSED ASSUMPTION (pending Hamza's sign-off): a v0 set — *sortir, manger, sport, ciné, parler, aider, jouer, voyager, boire un truc, se voir, travailler, autre* — is proposed unchanged from the retired matching-era list, since narrowing the use does not require narrowing the set. |
| PRO-04 | Pre-send review shows every currently-included recipient (« rien n'est masqué en silence », product law 2) — **except** anyone excluded by the FLT-02 veto absolu, who appears in neither an included nor an excluded list, with no override control anywhere (unchanged FLT-02 semantics). No rule-level "filtered" column exists — FS-06's survival and shape are decided in [SUG-SPEC-018](../../suggestions/specs/SUG-SPEC-018-adr002-fs06-survival.md) (OQ-PRO-7, resolved: option B, FLT-02 survives verbatim, the rest is retired); this row states only the FS-05-facing effect, and defers to whatever FS-06 becomes for FLT-02's own definition. |
...
| PRO-06 | Propositions expire (default 48h). Confirmation copy **« Elle expire dans 48 heures. »** stays frozen (SUG-DES-010, resolved 2026-08-09). Expiry is **not** invisible to recipients — a proposition is visible to them from the moment it arrives, so they see it age and eventually disappear. |
...
`POST /propositions` (verb, category?, expiresAt, recipientIds[]) → `201` — a saved group is resolved
server-side into this same `recipientIds` shape before validation (`PRO-02`, `PRO-09`); the client never
assembles a group's membership itself. `DELETE /propositions/:id` → withdraw (author only, `PRO-05`).
...
| ENV-03 | Reframe → PRO-04 | Pre-send review survives as « rien n'est masqué en silence » (law 2); the rule-level « Filtrés par tes règles » column depends on FS-06's fate (OQ-PRO-7, resolved B — see [SUG-SPEC-018](../../suggestions/specs/SUG-SPEC-018-adr002-fs06-survival.md)). |
...
- **OQ-ENV-2** — default expiry 48h (`PRO-06`, confirmed) vs same-day-midnight semantics — **carries
  ...
  the matching-engine questions; [SUG-SPEC-016](../../suggestions/specs/SUG-SPEC-016-adr002-fs05-rewrite.md)'s
...
- **OQ-ENV-3** — should `POST /propositions` enforce `recipientIds ⊆ author's ContactLink targets`
  (FS-07 edges)? **Carries and sharpens.** With server-side group resolution (`PRO-09`) the server now
  sees more than it did under the retired `ENV-05`, so this trade-off should be re-argued rather than
  assumed settled by the pivot. Not decided here — Architect resolves with Hamza.
