...
**Layout (reorganised 2026-08-16):** implemented suggestions move to `done/<area>/`; `<area>/` holds only what is still open. Nothing is ever deleted — a completed suggestion stays as the audit record of why the change was made. **77 done, 39 open.**

> ⚠️ These counts track *files*, and several merged suggestions were never moved (`SUG-IOS-004/005/007/012`, `SUG-AND-012` are all merged but still sit under their open `<area>/` folder), so "open" currently overstates by ~5. The move is each PR's own bookkeeping and is explicitly permitted by the scope guard (`SHARED_ALLOWED_PREFIXES` includes `suggestions/`); it was simply skipped. Worth one sweep to reconcile. Links throughout this file point at wherever the file now lives.
...
|---|---|---|---|
| iOS | [11](ios/) | [7](done/ios/) | ios-specialist |
| Android | [4](android/) | [14](done/android/) | android-specialist |
